...
export const verifyZkAddress = (signature, userAddress) => {
    ...
    if (parsedSignature.signatureScheme == "ZkLogin") {
        const legacyAddress = computeZkLoginAddressFromSeed(
            parsedSignature.zkLogin.addressSeed,
            parsedSignature.zkLogin.iss,
            true
        );
        const nonLegacyAddress = computeZkLoginAddressFromSeed(
            parsedSignature.zkLogin.addressSeed,
            parsedSignature.zkLogin.iss,
            false
        );
...
export const verifyZkPublicKey = ({ signature, publicKey }) => {
    const parsedSignature = parseSerializedSignature(signature);
    if (parsedSignature.signatureScheme == "ZkLogin") {
        const userSignature = toB64(parsedSignature.zkLogin.userSignature as any);
