...
import { ClmmpoolsError, MathErrorCode } from "./errors";
...
export class MathUtil {
    ...
    static checkUnsignedSub(n0: BN, n1: BN): BN {
        ...
        if (n.isNeg()) {
            throw new ClmmpoolsError(
                "Unsigned integer subtraction resulted in an overflow",
    ...
    static checkMulDivFloor(n0: BN, n1: BN, denom: BN, limit: number): BN {
        if (denom.eq(ZERO)) {
            throw new ClmmpoolsError("Divide by 0", MathErrorCode.DivideByZero);
        }
        const n = n0.mul(n1).div(denom);
        if (this.isOverflow(n, limit)) {
            throw new ClmmpoolsError(
                "Multiplication resulted in a division overflow",
    ...
    static checkMulShiftRight(n0: BN, n1: BN, shift: number, limit: number): BN {
        ...
        if (this.isOverflow(n, limit)) {
            throw new ClmmpoolsError(
                "Multiplication resulted in a shift right overflow",
    ...
    static checkMulShiftLeft(n0: BN, n1: BN, shift: number, limit: number): BN {
        ...
        if (this.isOverflow(n, limit)) {
            throw new ClmmpoolsError(
                "Multiplication resulted in a shift left overflow",
