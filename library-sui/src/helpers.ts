...
import { Keypair, Ed25519Keypair } from "./types";
...
export function getTestAccounts(create = false): Account[] {
    ...
    if (create) {
        ...
        for (const wallet of TEST_WALLETS) {
            const signer = getSignerFromSeed(wallet.phrase, "ED25519");
            const keyPair = getKeyPairFromSeed(wallet.phrase, "ED25519");
...
export function processTradingStartTime(
    ...
    const threshold = 1000; // perpetual will start trading after 1 second
