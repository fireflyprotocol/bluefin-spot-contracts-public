...
module bluefin_spot::clmm_math {
    ...
    public fun get_amount_by_liquidity(
        ...
        let (amount_a, amount_b) = if (i32H::lt(current_tick_index, tick_lower)) {
            (get_delta_a(lower_price, upper_price, liquidity, round_up), 0)
        } else if (i32H::lt(current_tick_index, tick_upper)) {
            (
    ...
    public fun get_liquidity_by_amount(
        ...
        if (is_fixed_a) {
            amount_a = amount;
            if (i32H::lt(current_tick_index, lower_index)) {
                _liquidity = get_liquidity_from_a(lower_price, upper_price, amount, false);
            }else if (i32H::lt(current_tick_index, upper_index)) {
            ...
            amount_b = amount;
            if (i32H::gte(current_tick_index, upper_index)) {
                _liquidity = get_liquidity_from_b(lower_price, upper_price, amount, false);
            }else if (i32H::gte(current_tick_index, lower_index)) {
    ...
    fun test_get_amount_by_liquidity_with_little_liquidity() {
        let (amount_a, amount_b) = get_amount_by_liquidity(
            i32H::lib_to_mate(i32::neg_from(1)),
            i32H::lib_to_mate(i32::from(1)),
            i32H::lib_to_mate(i32::from(0)),
            tick_math::get_sqrt_price_at_tick(i32H::lib_to_mate(i32::from(0))),
            1,
            true
        );
        ...
        let (amount_a, amount_b) = get_amount_by_liquidity(
            i32H::lib_to_mate(i32::neg_from(1)),
            i32H::lib_to_mate(i32::from(1)),
            i32H::lib_to_mate(i32::from(200)),
            tick_math::get_sqrt_price_at_tick(i32H::lib_to_mate(i32::from(200))),
            1,
            true
        );
        assert!(amount_a > 0 || amount_b > 0, 0);
        ...
        let (amount_a, amount_b) = get_amount_by_liquidity(
            i32H::lib_to_mate(i32::neg_from(1)),
            i32H::lib_to_mate(i32::from(1)),
            i32H::lib_to_mate(i32::neg_from(200)),
            tick_math::get_sqrt_price_at_tick(i32H::lib_to_mate(i32::neg_from(200))),
            1,
            true
        );
        assert!(amount_a > 0 || amount_b > 0, 0);
    ...
    fun test_get_liquidity_by_amount_with_little_amount() {
        let (liquidity, amount_a, amount_b) = get_liquidity_by_amount(
            i32H::lib_to_mate(i32::neg_from(443636)),
            i32H::lib_to_mate(i32::from(443636)),
            i32H::lib_to_mate(i32::from(0)),
            tick_math::get_sqrt_price_at_tick(i32H::lib_to_mate(i32::from(0))),
            1,
            true
